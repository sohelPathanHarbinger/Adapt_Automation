...
export default class NotifyPopupView extends Backbone.View {
  ...
  cancelNotify() {
    ...
    let notifyClosingComponentId = this.model.get("_id");
    let questionModel = Adapt.findById(notifyClosingComponentId);
    if (questionModel.get("_isQuestionType")) {
      const view = Adapt.findViewByModelId(notifyClosingComponentId);
      if (!questionModel.get("_isCorrect") && questionModel.get("_attemptsLeft")> 0) {
        view.onResetClicked();
      }
    }
  }
